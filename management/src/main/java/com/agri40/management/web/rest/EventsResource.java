...
/**
 * REST controller for managing {@link com.agri40.management.domain.Events}.
 */
@RestController
@RequestMapping("/api")
public class EventsResource {
    ...
    private static final String ENTITY_NAME = "managementEvents";
    ...
    public EventsResource(EventsRepository eventsRepository, CowRepository cowRepository) {
    ...
    public ResponseEntity<Events> createEvents(@RequestBody Events events) throws URISyntaxException {
        ...
        if (events.getType() == EventType.CHALEUR) {
            if (!cowRepository.existsById(events.getCowId())) {
    ...
    public ResponseEntity<Events> updateEvents(@PathVariable(value = "id", required = false) final String id, @RequestBody Events events)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, events.getId()))
            .body(result);
    }
    ...
    public ResponseEntity<Events> partialUpdateEvents(
        ...
        Optional<Events> result = eventsRepository
            ...
            .map(existingEvents -> {
                ...
                if (events.getCowId() != null) {
                    existingEvents.setCowId(events.getCowId());
                }
        ...
        return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, events.getId()));
    }
     ...
     */
    @GetMapping("/events")
    public ResponseEntity<List<Events>> getAllEvents(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
